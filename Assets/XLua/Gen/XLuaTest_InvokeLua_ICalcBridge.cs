...
namespace XLua.CSObjectWrap
{
    public class XLuaTestInvokeLuaICalcBridge : LuaBase, XLuaTest.InvokeLua.ICalc
    {
		...
		event System.EventHandler XLuaTest.InvokeLua.ICalc.EventTest
		{
			add
			{
#if THREAD_SAFE || HOTFIX_ENABLE
				lock (luaEnv.luaEnvLock)
				{
					...
					LuaAPI.lua_getref(L, luaReference);
					LuaAPI.xlua_pushasciistring(L, "add_EventTest");
					if (0 != LuaAPI.xlua_pgettable(L, -2))
					{
						luaEnv.ThrowExceptionFromError(err_func - 1);
					}
					if(!LuaAPI.lua_isfunction(L, -1))
					{
						LuaAPI.xlua_pushasciistring(L, "no such function add_EventTest");
						luaEnv.ThrowExceptionFromError(err_func - 1);
					}
					LuaAPI.lua_pushvalue(L, -2);
					LuaAPI.lua_remove(L, -3);
			...
			remove
			{
				...
				lock (luaEnv.luaEnvLock)
				{
					...
					LuaAPI.lua_getref(L, luaReference);
					LuaAPI.xlua_pushasciistring(L, "remove_EventTest");
					if (0 != LuaAPI.xlua_pgettable(L, -2))
					{
						luaEnv.ThrowExceptionFromError(err_func - 1);
					}
					if(!LuaAPI.lua_isfunction(L, -1))
					{
						LuaAPI.xlua_pushasciistring(L, "no such function remove_EventTest");
						luaEnv.ThrowExceptionFromError(err_func - 1);
					}
					LuaAPI.lua_pushvalue(L, -2);
					LuaAPI.lua_remove(L, -3);
